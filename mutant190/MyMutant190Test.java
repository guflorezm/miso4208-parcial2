...
public class MyMutant190Test {

    private static final String ACCOUNTS_TEXT = "Accounts";
    private static final String EDIT_TRANSACTION_TEXT = "Edit Transaction";
    ...
    public void myMutant190Test() {
        ...
        ViewInteraction editAction = onView(withId(R.id.edit_transaction));
        editAction.perform(click());

        ViewInteraction editTransactionText = onView( allOf(withParent(withId(R.id.toolbar)), withText(EDIT_TRANSACTION_TEXT), isDisplayed()));
        editTransactionText.check(matches(withText(EDIT_TRANSACTION_TEXT)));

        ViewInteraction noteEditText = onView(withId(R.id.input_description));
        noteEditText.perform(scrollTo(), replaceText("This is the salary of the month edited"), closeSoftKeyboard());
